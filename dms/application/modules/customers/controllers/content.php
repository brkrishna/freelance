...
class content extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Customers.Content.View');
	...
	public function index()
	{
        ...
        $total = $this->customers_model->count_all();
        ...
        Template::set('records', $this->customers_model->limit($limit,$offset)->find_all());
	...
	{
		$this->auth->restrict('Customers.Content.Create');
		...
		if (isset($_POST['save']))
		{
			if ($insert_id = $this->save_customers())
			{
				...
				redirect(SITE_AREA .'/content/customers');
			}
			else
			{
	...
	public function edit()
	{
		...
		if (isset($_POST['save']))
		{
			$this->auth->restrict('Customers.Content.Edit');
		...
		{
			$this->auth->restrict('Customers.Content.Delete');
	...
	private function save_customers($type='insert', $id=0)
	{
		...
		$data['contact_name']        = $this->input->post('customers_contact_name');
		$data['contact_phone']        = $this->input->post('customers_contact_phone');
		$data['contact_email']        = $this->input->post('customers_contact_email');
		$data['address1']        = $this->input->post('customers_address1');
		$data['address2']        = $this->input->post('customers_address2');
		$data['city']        = $this->input->post('customers_city');
		$data['country']        = $this->input->post('customers_country');
		$data['work_phones']        = $this->input->post('customers_work_phones');
		$data['website_url']        = $this->input->post('customers_website_url');
