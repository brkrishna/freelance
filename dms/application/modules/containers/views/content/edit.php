		...
		<fieldset>
			...
			<?php // Change the values in this array to populate your dropdown as required
				if (is_array($products_select) && count($products_select)) :	
					echo form_dropdown('containers_product_id', $products_select, set_value('containers_product_id', isset($containers['product_id']) ? $containers['product'] : ''), 'Product'. lang('bf_form_label_required'));
				endif;
			?>
			...
			<div class="form-actions">
				<input type="submit" name="save" class="btn btn-primary" value="<?php echo lang('containers_action_edit'); ?>"  />
				<?php echo lang('bf_or'); ?>
				<?php echo anchor(SITE_AREA .'/content/containers', lang('containers_cancel'), 'class="btn btn-warning"'); ?>
				
			<?php if ($this->auth->has_permission('Containers.Content.Delete')) : ?>
				or
				<button type="submit" name="delete" class="btn btn-danger" id="delete-me" onclick="return confirm('<?php e(js_escape(lang('containers_delete_confirm'))); ?>'); ">
					<span class="icon-trash icon-white"></span>&nbsp;<?php echo lang('containers_delete_record'); ?>
				</button>
			<?php endif; ?>
			</div>
		</fieldset>
