...
#-------------------------------------------------------------------------------
# Name:         vmi
# Purpose:      Virginia Military Institute
#
# Author:       Ramakrishna
#
# Dated:        14/Apr/2016
...
url = 'http://www9.vmi.edu/directories/Directory-Cadets.asp'
...
def search(term):
	try:
		s = requests.session()
		data = {'B1':'Search','formaction':'getlist','alias':term.replace("\n","")}
		r = s.post(url, headers=headers, data=data)
		tree = html.fromstring(r.content)

		students = tree.xpath("//div[@id='Friendly']/table//tr")
		...
		if len(records) > 0:
			file_exists = os.path.isfile('vmi.csv')
			with open('vmi.csv', 'a', newline='', encoding='utf-8') as outfile:
				fp = csv.DictWriter(outfile, records[0].keys())
				if not file_exists:
					fp.writeheader()
				fp.writerows(records)

		with open('vmi_terms', 'a') as f:
			f.write(term)
...
def main():
	try:
		...
		if os.path.isfile('vmi_terms'):
			finished_terms = set(open('vmi_terms').readlines())
